...
- **测试环境**:
  ...
  - Bun 1.3+
  - Node.js 22+
- **依赖服务**: MinIO（S3 兼容对象存储；不可用时测试自动跳过）

## 测试结果

### 总体统计

- **总测试数**: 115（Deno）/ 105（Bun）/ 105（Node）
- **通过**: 115 / 105 / 105 ✅
- **失败**: 0
- **通过率**: 100% ✅
- **测试执行时间**: Deno ~0.5 秒 / Bun ~0.07 秒 / Node ~0.35 秒

### 三端测试摘要

三端全部通过。Deno 计 115 条（含 10 条 Deno runner 注入的 `@dreamer/test
cleanup browsers`，每个测试文件一条）；Bun 与 Node 各计 105 条。云存储适配器测试
（S3/OSS/COS/分片上传）在 MinIO 不可用时（如 CI）自动跳过。
...
**测试报告生成时间**: 2026-01-30 **测试框架**: @dreamer/test@^1.0.0-beta.40
**测试环境**: Bun 1.3.5, Deno 2.6.4 **测试总数**: 107 **通过率**: 100% ✅
