...
> @dreamer/upload 的客户端模块。面向浏览器与 Deno/Bun
> 的通用文件上传客户端，支持分片上传、断点续传与进度回调。

**English**: [Client (EN)](../../en-US/client/README.md) · **返回主文档**:
[README (中文)](../README.md)
...
上传客户端与实现了分片上传 API（init → chunk → complete /
abort）的服务端配合使用，支持：
...
- **令牌**：通过 `setToken()` 或 `config.token` 设置，以
  `Authorization: Bearer <token>` 形式发送。
