/**
 * @fileoverview OSS 适配器测试
 *
 * 使用 MinIO（S3 兼容模式）测试 OSS 适配器功能
 *
 * 运行测试前确保 MinIO 已启动：
...
/** 测试文件内容 */
const TEST_CONTENT = new TextEncoder().encode("Hello, OSS via MinIO!");
const TEST_LARGE_CONTENT = new Uint8Array(10 * 1024 * 1024).fill(66); // 10MB 的 'B'
...
function generateTestKey(prefix: string): string {
  return `oss-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
}
...
async function createTestBucket(): Promise<void> {
  const adapter = createOSSAdapter(TEST_CONFIG);
  try {
    // 尝试上传一个测试文件来验证桶是否存在
    await adapter.upload("__test_oss__", new Uint8Array([1]));
    await adapter.delete("__test_oss__");
  } catch {
...
// ============================================================================
// OSS 适配器测试（使用 MinIO S3 兼容模式）
// ============================================================================

describe("OSS 适配器测试（MinIO S3 兼容模式）", () => {
  // 测试前检查 MinIO 是否可用
  beforeAll(async () => {
    minioAvailable = await checkMinioAvailable();
    if (minioAvailable) {
      await createTestBucket();
      console.log("✅ MinIO 可用，开始 OSS 适配器测试");
    } else {
  ...
  describe("基础功能", () => {
    ...
    it("应该能列出对象", async () => {
      ...
      const adapter = createOSSAdapter(TEST_CONFIG);
      const prefix = `oss-list-test-${Date.now()}`;
      const keys = [
        `${prefix}/file1.txt`,
        `${prefix}/file2.txt`,
        `${prefix}/file3.txt`,
    ...
    it("应该能生成预签名 URL", async () => {
      ...
      // 生成预签名 URL（OSS 原生签名，S3 兼容模式下可能不工作）
      // 注意：S3 兼容模式下预签名 URL 使用的是 OSS 原生签名
